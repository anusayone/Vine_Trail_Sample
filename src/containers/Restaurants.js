...
class Restaurants extends Component {
    ...
    onBackPress = () => {
        console.log("back button of res listing page");
    ...
    componentWillUnmount() {
        console.log("componentWillUnmount res");
        ...
        this.watchId = navigator.geolocation.getCurrentPosition(
                    (position) => {
                        ...
                        if (this.currentLatitude != '') {
                            this.props.getMoreRestaurants(17, this.currentLatitude, this.currentLongitude).then(() => {
                                this.setState({
    ...
    fetchMoreRestaurants() {
        if (this.props.restaurants.restaurants.next) {
            ...
            this.props.getMoreRestaurants(17, this.currentLatitude, this.currentLongitude, true).then(() => {
                this.setState({
                    restaurants: this.props.restaurants.restaurants.results,
                    isRestaurantsAvailable: true,
                    isMoreLoaderVisible:false
    ...
    searchPoi(term) {
        ...
        this.searchTimeOutId = setTimeout(() => {
            if (term.length > 2) {
                that.props.getCategorySearchResults(17,term).then(() => {
    ...
    render() {
        ...
        let lastDistance;
                 if(restaurants&&this.props.explore.distance[restaurants[2].id]){
                        if(this.props.explore.distance[restaurants[2].id]["0"].elements["0"].status=='OK'){
                                         lastDistance="more than "+this.props.explore.distance[restaurants[2].id]["0"].elements["0"].distance.text;
                                         //console.log(this.props.explore.distance[restaurants[2].id]["0"].elements["0"].distance.text)
                                        }else{
                                        //console.log("hhh")
                                        lastDistance=this.props.explore.distance[restaurants[2].id]["0"].elements["0"].status;
                                        }
                                        }else{
                                        //console.log("l")
                                        }
                        ...
                        (this.state.isRestaurantsAvailable)
                            ? <FlatList
                                ...
                                renderItem={({item,index}) =>
                                    <POIComponent
                                        item={item}
                                        distance={(this.props.explore.distance[item.id]&&this.props.explore.distance[item.id]["0"].elements["0"].status=='OK')
