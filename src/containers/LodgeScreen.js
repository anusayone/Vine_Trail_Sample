...
class LodgeScreen extends Component {
       ...
       this.watchId = navigator.geolocation.getCurrentPosition(
                   (position) => {
                       ...
                       if (this.currentLatitude != '') {
                           this.props.getMoreRestaurants(18,this.currentLatitude,this.currentLongitude).then(() => {
                               this.setState({
    ...
    onBackPress = () => {
        console.log("back button of lodges listing page");
    ...
    onSelectedPOI(item) {
        this.props.navigation.navigate(Types.RESTAURANTS_DETAIL, item);
    }
    ...
    fetchMoreRestaurants(){
        if (this.props.restaurants.restaurants.next) {
            ...
            this.props.getMoreRestaurants(18, this.currentLatitude, this.currentLongitude, true).then(() => {
                this.setState({
                    restaurants: this.props.restaurants.restaurants.results,
                    isRestaurantsAvailable: true,
                    isMoreLoaderVisible:false
    ...
    searchPoi(term) {
        ...
        this.searchTimeOutId = setTimeout(() => {
            if (term.length > 2) {
                that.props.getCategorySearchResults(18,term).then(() => {
    ...
    onSearchItemSelected(selectedPOI) {
        ...
        this.props.navigation.navigate(Types.RESTAURANTS_DETAIL, selectedPOI);
    }

    render() {
        let restaurants = this.props.restaurants.restaurants.results;
        let lastDistance;
                 if(restaurants&&this.props.explore.lodgesDistance[restaurants[2].id]){
                          if(this.props.explore.lodgesDistance[restaurants[2].id]["0"].elements["0"].status=='OK'){
                                  lastDistance="more than "+this.props.explore.lodgesDistance[restaurants[2].id]["0"].elements["0"].distance.text;
                                  //console.log(this.props.explore.distance[restaurants[2].id]["0"].elements["0"].distance.text)
                                 }else{
                                 //console.log("hhh")
                                 lastDistance=this.props.explore.lodgesDistance[restaurants[2].id]["0"].elements["0"].status;
                                 }
                                 }else{
                                 //console.log("l")
                                 }
                        ...
                        (this.state.isRestaurantsAvailable)
                            ? <FlatList
                                ...
                                renderItem={({item,index}) =>
                                    <POIComponent
                                        item={item}
                                        distance={(this.props.explore.lodgesDistance[item.id]&&this.props.explore.lodgesDistance[item.id]["0"].elements["0"].status=='OK')
                                    ...
                                    />}
                                //keyExtractor={(item, index) => 'l'+item.id.toString()}
                                onEndReached={()=>{
...
export default connect(mapStateToProps, mapDispatchToProps)(LodgeScreen);
